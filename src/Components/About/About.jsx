import { useEffect, useRef } from "react";
import { motion, useInView, useAnimation } from "framer-motion";
import { HiMiniCodeBracket } from "react-icons/hi2";
import { FaGraduationCap } from "react-icons/fa6";
import { MdOutlineWorkOutline } from "react-icons/md";
import Skills from "../Skills/Skills";
import "./about.scss";

const About = () => {
  return (
    <section id="about">
      <div className="container">
        <AboutHeader />
        <AboutContent />
      </div>
    </section>
  );
};

const AboutHeader = () => {
  const controls = useAnimation();
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.3 });

  useEffect(() => {
    if (isInView) {
      controls.start("visible");
    }
  }, [controls, isInView]);

  return (
    <motion.div
      className="about-header"
      ref={ref}
      initial="hidden"
      animate={controls}
      variants={{
        hidden: { opacity: 0, y: 50 },
        visible: { opacity: 1, y: 0, transition: { duration: 0.6 } },
      }}
    >
      <h2 className="section-heading">
        <motion.span
          className="accent-text"
          whileHover={{ scale: 1.05 }}
          transition={{ type: "spring", stiffness: 300 }}
        >
          Engineering
        </motion.span>{" "}
        Ideas <br />
        into Reality...
      </h2>
    </motion.div>
  );
};

const AboutContent = () => {
  return (
    <div className="about-content">
      <AboutImage />
      <AboutInfo />
    </div>
  );
};

const AboutImage = () => {
  const controls = useAnimation();
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.3 });

  useEffect(() => {
    if (isInView) {
      controls.start("visible");
    }
  }, [controls, isInView]);

  return (
    <motion.div
      className="about-image"
      ref={ref}
      initial="hidden"
      animate={controls}
      variants={{
        hidden: { opacity: 0, x: -50 },
        visible: {
          opacity: 1,
          x: 0,
          transition: {
            duration: 0.8,
            ease: "easeOut",
          },
        },
      }}
    >
      <motion.div
        className="image-container"
        whileHover={{ scale: 1.03 }}
        transition={{ duration: 0.3 }}
      >
        <motion.img
          src="./grid1.png"
          alt="profile"
          initial={{ filter: "blur(5px)" }}
          animate={{ filter: "blur(0px)" }}
          transition={{ duration: 1.2, delay: 0.3 }}
        />
        <motion.div
          className="image-decoration"
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.8, delay: 0.5 }}
        />
      </motion.div>
    </motion.div>
  );
};

const AboutInfo = () => {
  const controls = useAnimation();
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.2 });

  useEffect(() => {
    if (isInView) {
      controls.start("visible");
    }
  }, [controls, isInView]);

  return (
    <motion.div
      className="about-info"
      ref={ref}
      initial="hidden"
      animate={controls}
      variants={{
        hidden: { opacity: 0 },
        visible: {
          opacity: 1,
          transition: { staggerChildren: 0.2, delayChildren: 0.3 },
        },
      }}
    >
      <AboutDescription />
      <AboutHighlights />
      <motion.div
        variants={{
          hidden: { opacity: 0, y: 20 },
          visible: {
            opacity: 1,
            y: 0,
            transition: { duration: 0.6, delay: 0.6 },
          },
        }}
      >
        <Skills />
      </motion.div>
    </motion.div>
  );
};

const AboutDescription = () => {
  return (
    <motion.div
      className="about-description"
      variants={{
        hidden: { opacity: 0, y: 20 },
        visible: { opacity: 1, y: 0, transition: { duration: 0.6 } },
      }}
    >
      <motion.h2
        className="section-subheading"
        variants={{
          hidden: { opacity: 0, y: 20 },
          visible: { opacity: 1, y: 0, transition: { duration: 0.6 } },
        }}
      >
        I'm a Backend Engineer based in India, with nearly two years of
        experience building scalable backend systems.
      </motion.h2>

      <motion.p
        className="description-text"
        variants={{
          hidden: { opacity: 0, y: 20 },
          visible: {
            opacity: 1,
            y: 0,
            transition: { duration: 0.6, delay: 0.1 },
          },
        }}
      >
        I started my journey at Infosys, where I worked on high-performance
        applications in the logistics domain, fine-tuned databases, and
        developed efficient APIs. Debugging, optimizing performance, and
        ensuring seamless backend operations was a big part of my work.
      </motion.p>

      <motion.p
        className="description-text"
        variants={{
          hidden: { opacity: 0, y: 20 },
          visible: {
            opacity: 1,
            y: 0,
            transition: { duration: 0.6, delay: 0.2 },
          },
        }}
      >
        These days, I'm focused on learning and exploring new technologies to
        build even better, more scalable solutions. I'm always excited to take
        on new challenges and make a real impact with my work.
      </motion.p>
    </motion.div>
  );
};

const HighlightItem = ({ icon, title, description, delay }) => {
  return (
    <motion.div
      className="highlight-item"
      variants={{
        hidden: { opacity: 0, y: 20 },
        visible: {
          opacity: 1,
          y: 0,
          transition: { duration: 0.5, delay },
        },
      }}
      whileHover={{
        y: -8,
        boxShadow: "0 10px 20px rgba(0, 0, 0, 0.1)",
      }}
      transition={{ duration: 0.3 }}
    >
      <motion.div
        className="highlight-icon"
        whileHover={{
          rotate: [0, -10, 10, -5, 5, 0],
          transition: { duration: 0.5 },
        }}
      >
        {icon}
      </motion.div>
      <h3 className="highlight-title">{title}</h3>
      <p className="highlight-description">{description}</p>
    </motion.div>
  );
};

const AboutHighlights = () => {
  return (
    <motion.div
      className="about-highlights"
      variants={{
        hidden: { opacity: 0 },
        visible: { opacity: 1 },
      }}
    >
      <HighlightItem
        icon={<HiMiniCodeBracket />}
        title="Technologies"
        description="Java, Spring MVC, Spring Boot, Hibernate, REST APIs, SQL"
        delay={0.3}
      />
      <HighlightItem
        icon={<FaGraduationCap />}
        title="Education"
        description="B.Tech in Computer Science and Engineering"
        delay={0.4}
      />
      <HighlightItem
        icon={<MdOutlineWorkOutline />}
        title="Experience"
        description="Around 2 years of experience in Infosys"
        delay={0.5}
      />
    </motion.div>
  );
};

export default About;

{
  /* <div>
          <svg
            width="889"
            height="596"
            viewBox="0 0 889 596"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <g clipPath="url(#clip0_181_1068)">
              <path
                d="M387.457 438.674C311.494 481.449 354.27 586.913 379.345 591.338C398.658 594.746 462.149 592.792 492.921 591.338L731.134 415.074L591.745 395.899C548.97 395.899 448.228 404.454 387.457 438.674Z"
                fill="white"
              />
              <path
                d="M492.921 591.338C491.822 591.393 494.111 591.281 492.921 591.338ZM492.921 591.338C462.149 592.792 398.658 594.746 379.345 591.338C354.27 586.913 311.494 481.449 387.457 438.674C448.228 404.454 548.97 395.899 591.745 395.899L731.134 415.074L492.921 591.338Z"
                stroke="black"
                strokeWidth="2.91102"
              />
              <path
                d="M408.106 453.152C460.616 443.122 503.244 473.965 517.994 490.64C520.458 487.195 523.122 483.789 525.998 480.44C521.464 460.527 433.181 439.14 408.106 453.152Z"
                fill="#000001"
              />
              <path
                d="M487.596 593.52C489.46 593.439 491.239 593.358 492.92 593.278L687.486 449.312C678.915 446.413 656.823 437.494 637.029 425.009C502.733 440.923 481.451 543.981 487.596 593.52Z"
                fill="#0E1729"
              />
              <path
                d="M709.745 593.277C806.357 562.302 731.132 422.176 731.132 417.014L694.994 412.042C510.913 423.405 483.576 537.6 492.918 593.277L523.984 849.191H604.037L610.919 593.277H709.745Z"
                fill="white"
                stroke="black"
                strokeWidth="2.91102"
              />
              <path
                d="M591.745 540.178C603.545 551.978 609.446 580.495 610.921 593.278H621.983C619.033 578.774 608.855 547.848 591.745 540.178Z"
                fill="#000001"
              />
              <rect x="94" y="593" width="796" height="3" fill="black" />
              <path
                d="M606.714 146.088C602.83 141.143 592.73 125.958 583.407 104.77C585.526 102.297 589.551 94.8104 588.704 84.6398C577.686 82.9447 571.009 59.2134 569.048 47.5596C583.679 39.4435 571.212 17.7929 575.991 16.8366C581.288 15.7766 595.061 21.0738 595.061 34.8464C595.061 48.619 618.657 42.118 624.725 39.0841C639.556 31.6681 634.26 49.6796 627.903 66.63C624.513 72.5628 636.378 83.9341 642.735 88.8781C644.429 105.829 629.315 131.962 621.546 142.909C622.606 174.692 622.818 238.258 615.19 238.258C610.105 228.088 607.421 172.573 606.714 146.088Z"
                fill="white"
                stroke="black"
                strokeWidth="1.94068"
              />
              <path
                d="M597.177 49.6725C595.411 51.4383 592.939 56.4529 597.177 62.3858"
                stroke="#241715"
                strokeWidth="0.970341"
              />
              <path
                d="M605.652 65.5718C604.946 69.4564 601.838 75.3186 595.058 67.6907"
                stroke="#241715"
                strokeWidth="0.970341"
              />
              <path
                d="M628.96 52.8538C630.372 48.9692 632.138 41.4119 627.9 42.2594"
                stroke="#241715"
                strokeWidth="0.970341"
              />
              <path
                d="M587.572 96.2916C588.506 93.0569 589.075 89.1066 588.702 84.6383C594.573 87.817 604.592 88.8765 625.781 74.0435C626.627 85.0616 600.661 93.4664 587.572 96.2916Z"
                fill="black"
              />
              <path
                d="M641.674 16.8356C649.09 34.846 632.139 59.213 627.901 66.629C634.258 49.6786 638.495 31.6671 624.723 39.0832C618.75 42.2994 595.059 48.6181 595.059 34.8454C595.059 21.0728 581.286 15.7756 575.989 16.8356C571.21 17.7919 583.677 39.4426 569.047 47.5587C567.46 48.4386 565.556 49.1594 563.276 49.6775C568.573 42.2615 566.454 43.3209 562.216 25.3105C557.979 7.30016 577.049 3.06296 595.059 0.944096C613.069 -1.17477 634.258 -1.17477 641.674 16.8356Z"
                fill="black"
              />
              <path
                d="M741.26 131.256C727.699 113.457 669.924 95.5883 642.732 88.8785C644.427 105.829 629.313 131.962 621.544 142.91C622.604 174.693 622.815 238.259 615.188 238.259C610.102 228.088 607.418 172.574 606.712 146.088C602.827 141.144 592.728 125.959 583.405 104.77C581.286 111.127 524.076 154.563 514.541 168.336C505.006 182.109 501.828 314.538 501.828 326.192C501.828 337.846 497.59 417.303 532.552 417.303C560.521 417.303 674.869 421.188 732.784 422.6C741.26 332.901 754.82 149.054 741.26 131.256Z"
                fill="black"
              />
              <path
                d="M501.831 326.185C559.747 314.178 675.578 300.971 675.578 344.196C675.578 387.42 621.9 411.849 595.061 418.659M675.578 98.3068C668.515 101.518 653.542 116.629 650.152 151.379C645.914 194.815 632.311 209.647 621.291 209.647M610.477 211.766C601.454 216.71 581.712 220.454 574.932 195.875C566.457 165.151 548.774 148.2 529.011 153.497M707.361 110.061C705.242 109.165 671.341 132.309 668.162 185.28C664.984 238.252 631.082 337.839 561.159 284.867C534.672 258.381 509.151 241.784 504.865 243.549M583.408 418.339C592.942 409.163 609.47 387.844 599.299 375.979C589.128 364.113 533.497 380.216 506.953 389.751M746.348 158.795C723.464 172.92 681.299 217.911 695.708 284.867C708.421 323.007 731.653 319.122 741.68 312.413M737.847 364.325C724.153 364.505 698.038 376.254 703.124 421.805"
                stroke="white"
                strokeWidth="0.970341"
              />
              <path
                d="M615.188 417.293V239.308"
                stroke="#027269"
                strokeWidth="0.970341"
              />
              <path
                d="M615.824 415.56L610.617 376.847C598.777 378.995 574.119 381.076 555.071 382.881C535.13 384.771 530.105 395.241 543.49 398.585C536.489 397.711 528.65 407.364 539.822 411.686C531.627 414.597 547.836 417.845 555.782 417.861C551.234 418.548 543.349 419.815 544.421 424.634C545.493 429.452 592.469 420.592 615.824 415.56Z"
                fill="white"
              />
              <path
                d="M615.824 415.561L610.617 376.848C607.863 377.348 604.415 377.844 600.505 378.334L588.66 420.952C598.612 419.142 608.409 417.159 615.824 415.561Z"
                fill="black"
              />
              <path
                d="M556.106 418.146L572.58 413.509"
                stroke="black"
                strokeWidth="0.970341"
              />
              <path
                d="M543.596 407.801L572.476 404.295"
                stroke="black"
                strokeWidth="0.970341"
              />
              <path
                d="M543.491 398.584L572.988 393.482"
                stroke="black"
                strokeWidth="0.970341"
              />
              <path
                d="M772.692 381.565C775.255 340.991 778.054 136.522 732.286 125.5C679.382 112.759 680.076 307.755 703.043 352.24L595.016 363.222L605.152 430.623C657.673 426.955 768.449 448.703 772.692 381.565Z"
                fill="black"
              />
              <path
                d="M732.285 125.499C679.382 112.759 680.075 307.754 703.043 352.24L595.015 363.222"
                stroke="white"
                strokeWidth="0.970341"
              />
              <path
                d="M754.322 154.883C737.975 156.661 701.645 166.099 692.733 188.221M764.493 190.481C749.761 196.925 715.271 218.617 710.92 240.885C706.57 263.152 752.646 255.634 771.274 251.505M772.404 290.493C756.92 297.722 702.644 302.52 690.473 285.973M772.968 359.993C757.111 350.37 711.46 331.161 699.514 338.522M656.571 356.603C649.79 371.294 660.409 425.315 690.473 431.189"
                stroke="white"
                strokeWidth="0.970341"
              />
              <path
                d="M309.324 279.458H491.26C494.447 279.458 497.311 281.407 498.481 284.372L552.515 421.342C553.685 424.307 556.548 426.256 559.736 426.256H701.251C703.854 426.256 705.964 428.366 705.964 430.969C705.964 433.573 703.854 435.683 701.251 435.683H365.118C361.935 435.683 359.075 433.74 357.902 430.782L302.107 290.082C300.087 284.986 303.842 279.458 309.324 279.458Z"
                fill="white"
                stroke="black"
                strokeWidth="2.91102"
              />
              <path
                d="M497.651 282.129L557.502 426.256H553.46C550.326 426.256 547.499 424.371 546.294 421.477L489.65 285.456C488.462 282.603 490.559 279.458 493.65 279.458C495.401 279.458 496.98 280.512 497.651 282.129Z"
                fill="black"
              />
              <path
                d="M357.902 430.782L302.107 290.082C300.087 284.986 303.842 279.458 309.324 279.458H323.352C324.507 279.458 325.643 279.716 326.692 280.199C331.658 282.486 339.954 286.082 353.731 293.043C399.822 316.331 405.644 365.818 395.941 390.077C388.074 409.743 492.053 430.071 532.905 435.683H365.118C361.935 435.683 359.075 433.74 357.902 430.782Z"
                fill="black"
              />
              <ellipse
                cx="11.5736"
                cy="13.3097"
                rx="11.5736"
                ry="13.3097"
                transform="matrix(-0.936312 0.351169 0.351169 0.936312 426.862 327.975)"
                fill="black"
              />
            </g>
            <defs>
              <clipPath id="clip0_181_1068">
                <rect width="889" height="596" fill="white" />
              </clipPath>
            </defs>
          </svg>
        </div> */
}
